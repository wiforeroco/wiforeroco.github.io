	...
	protected function get_instagram_data_with_token() {
		...
		if ( 'business' == $this->api_type ) {
			...
			if ( empty($this->username) || $this->username == $this->business_username ) {
				$response = wp_remote_get( 'https://graph.facebook.com/v7.0/' . $this->user_id . '/media?fields=media_url,thumbnail_url,caption,id,media_type,timestamp,username,comments_count,like_count,permalink,children{media_url,id,media_type,timestamp,permalink,thumbnail_url}&limit=12&access_token=' . $this->access_token );
			} else {
				$response = wp_remote_get( 'https://graph.facebook.com/v7.0/' . $this->user_id . '?fields=business_discovery.username('. $this->username .'){followers_count,media_count,media{media_url,media_type,caption,permalink,timestamp,comments_count,like_count}}&limit=12&access_token=' . $this->access_token );
			}

		} else {
			$response = wp_remote_get( 'https://graph.instagram.com/'. $this->user_id .'/media?fields=media_url,thumbnail_url,caption,id,media_type,timestamp,username,comments_count,like_count,permalink,children{media_url,id,media_type,timestamp,permalink,thumbnail_url}&limit=12&access_token='.$this->access_token );
		}
