	...
	class MultiPostThumbnails {
		...
		public function register($args = array()) {
			...
			if (null === $this->label || null === $this->id) {
				if (WP_DEBUG) {
					trigger_error(sprintf(esc_html__("The 'label' and 'id' values of the 'args' parameter of '%s::%s()' are required", 'signy'), __CLASS__, __FUNCTION__));
				}
				return;
			}
		...
		public function add_attachment_field($form_fields, $post) {
			...
			$link = sprintf('<a id="%4$s-%1$s-thumbnail-%2$s" class="%1$s-thumbnail" href="#" onclick="MultiPostThumbnails.setAsThumbnail(\'%2$s\', \'%1$s\', \'%4$s\', \'%5$s\');return false;">' . esc_html__( 'Set as %3$s', 'signy' ) . '</a>', $this->id, $post->ID, $this->label, $this->post_type, $ajax_nonce);
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			if (version_compare($wp_version, '3.5', '<')) {
				add_thickbox();
				wp_enqueue_script( "mpt-featured-image", get_template_directory_uri().'/inc/plugins/woocommerce/product-img-hover-thumbnails/multi-post-thumbnails-admin.js', array( 'jquery', 'media-upload' ) );
			} else { // 3.5+ media modal
				wp_enqueue_media( array( 'post' => ( $post_ID ? $post_ID : null ) ) );
				wp_enqueue_script( "mpt-featured-image", get_template_directory_uri().'/inc/plugins/woocommerce/product-img-hover-thumbnails/multi-post-thumbnails-admin.js', array( 'jquery', 'set-post-thumbnail' ) );
				wp_enqueue_script( "mpt-featured-image-modal", get_template_directory_uri().'/inc/plugins/woocommerce/product-img-hover-thunbmails/media-modal.js', array( 'jquery', 'media-models' ) );
			}
		...
		private function plugins_url($relative_path, $plugin_path) {
			...
			if(0 === strpos($plugin_path, $template_dir)) {
				...
				$folder = str_replace($template_dir, '', get_stylesheet_directory($plugin_path));
				if ( '.' != $folder ) {
					$url .= '/' . ltrim($folder, '/');
				}
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			$set_thumbnail_link = sprintf( $format_string, sprintf( esc_html__( "Set %s" , 'signy' ), $this->label ), esc_url($image_library_url), $this->post_type, $this->id, $url_class, $this->label, $thumbnail_id );
			$content = sprintf( $set_thumbnail_link, sprintf( esc_html__( "Set %s", 'signy' ), $this->label ) );
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
					...
					$content .= sprintf( $format_string, $this->post_type, $this->id, sprintf( esc_html__( "Remove %s", 'signy' ), $this->label ), $ajax_nonce );
				}
